...
import style from './cyberware.module.css';
...
export function CyberWare() {
  ...
  const toggleHandSelection = () => {
    setHandSelected(!handSelected);
  ...
  return (
    <section className="cyberpunk">
      ...
      <div
        className={`human-body ${handSelected ? 'hand-selected' : ''}`} 
        style={{ position: 'fixed', top: 0, left: 0, zIndex: 999 }}
        onClick={toggleHandSelection}
      >
        ...
        <svg
          data-position="hands"
          className={`hands ${handSelected ? 'hand-selected' : ''}`}
      ...
      {showSkeleton && (
        <div>
          <Skeleton characterData={characterData}/>
        </div>
      )}
    </section>
  );
